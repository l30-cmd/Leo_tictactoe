...
def checks_index(s, index):  #borrowed by https://stackoverflow.com/questions/41752946/replacing-a-character-from-a-certain-index
    ...
    if index not in range(len(s)):
        pass #raise ValueError("index outside given string")

    # if not erroring, but the index is still not in the correct range..
    if index < 0:  # add it to the beginning
        pass #raise ValueError("index outside given string - number to low") #adapted from the borrowed code

    if index > len(s):  # add it to the end
        pass #raise ValueError("index outside given string - to high of a number")
    else:
        valid_index = True
    return(valid_index)
...
def move(board4check, marku):
    ...
    while True:
        ...
        if checks_index(board4check, pos_u):
